...
class Membro:
	...
	def carregarDadosCVLattes(self):
		...
		if 'xml' in cvPath:
			...
			self.url      = parser.url
			print "(*) Utilizando CV xml armazenado no cache: "+cvPath

		elif '0000000000000000'==self.idLattes:
			# se o codigo for '0000000000000000' então serao considerados dados de pessoa estrangeira - aem Lattes. 
			# sera procurada a coautoria endogena com os outros membro.
			# para issoé necessario indicar o nome abreviado no arquivo .list
			return 

		else:
			if os.path.exists(cvPath):
				arquivoH = open(cvPath)
				#arquivoH = codecs.open(cvPath)
				#arquivoH = codecs.open(cvPath, "r", encoding='iso-8859-1', errors='ignore')
				cvLattesHTML = arquivoH.read()
				if self.idMembro!='':
					print "(*) Utilizando CV html armazenado no cache: "+cvPath
			else:
				cvLattesHTML = baixaCVLattes(self.idLattes)
				if not self.diretorioCache=='':
					file = open(cvPath, 'w')
					#file = codecs.open(cvPath, 'w')
			...
			# cvLattesHTML  = cvLattesHTML.decode('iso-8859-1',errors='ignore')+extended_chars+special_chars
